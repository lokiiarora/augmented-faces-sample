package com.lokesh.augmentedfacessample
...
class MainActivity : AppCompatActivity() {
    ...
    fun loadVariants() {
        ...
        chipGroup.setOnCheckedChangeListener { group, checkedId ->
            ...
            if (chipText != null) {
                ...
                materialMap.forEach { (_, mat) ->
                    Log.d("Color debug", "${colorInt.red}, ${colorInt.green}, ${colorInt.blue}, ${colorInt.alpha}")
                    mat.setParameter("baseColorFactor", colorInt.red / 255f, colorInt.green / 255f, colorInt.blue / 255f, colorInt.alpha / 255f)
                }
            }
        }
        val frecklesCheckbox = findViewById<CheckBox?>(R.id.use_sample_makeup)
        frecklesCheckbox?.setOnCheckedChangeListener { _, isChecked ->
            facesNodes.forEach { (_, node) ->
                node.faceMeshTexture = if (isChecked) faceTexture else null
            }
        }
    }
    ...
    private fun onNewFaceNodeAdded(faceNode: AugmentedFaceNode, renderableInstance: RenderableInstance) {
        ...
        fa?.let {
            ...
            it.materialInstances.forEachIndexed { index, materialInstance ->
                ...
                parameters.forEach { parameter ->
                    Log.d("Material Parameter", "name of parameter: ${parameter.name} type: ${parameter.type}")
                }
            }
        }
    }
